 ...
 */
export async function deviceLogin(config, onCode) {
    ...
    onCode({
        ...
        verificationUriComplete: typeof response.verification_uri_complete === "string"
            ? response.verification_uri_complete
            : undefined,
    });
    const tokens = await oidc.pollDeviceAuthorizationGrant(provider, response, undefined, { signal: AbortSignal.timeout((Number(response.expires_in) ?? 600) * 1000) });
...
export async function refreshAccessToken(config, tokens) {
    ...
    const updated = {
        ...tokens,
        accessToken: typeof refreshed.access_token === "string" ? refreshed.access_token : tokens.accessToken,
...
export async function validAccessToken(config) {
...
async function providerConfig(config) {
    const options = { timeout: 10 };
    const issuer = new URL(config.issuer);
    if (issuer.protocol === "http:") {
        options.execute = [oidc.allowInsecureRequests];
    }
    return oidc.discovery(issuer, config.cliClientId, { token_endpoint_auth_method: "none" }, oidc.None(), options);
}
