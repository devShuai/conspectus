...
async function main() {
    ...
    switch (command) {
        case "configure": {
            ...
            const cliClientId = (await ask("CLI client id (default conspectus-cli): ")) || "conspectus-cli";
        ...
        case "status": {
            const config = loadCliConfig();
            const collectors = await Promise.all(listCollectors().map(async (c) => ({
                id: c.id,
        ...
        default:
            console.log("Usage: conspectus-collect <configure|login|status|run [--dry-run]|logout>");
            process.exitCode = 1;
    }
}
main().catch((error) => {
    console.error("conspectus-collect:", error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
});
