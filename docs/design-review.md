...
| **R12-UPDATEDAT-FP**（**已闭环**，见修订记录 2026-08-09 / conspectus#125） | P1 · 可用性（**代码核对得出**） | `updated_at > emailSnapshotIssuedAt` 被用作"邮箱快照后画像变化"的判据，但 certus 的 `UpdatedAt` 在**任何**用户更新时都 bump（`internal/identity/user.go` 的 `Update` 无条件赋值），改显示名、改状态都算。一次无关的画像变更就会写 `emailSyncRequiredAt`，**静默延迟该用户的邮件通知直到下次登录**——本地会话最长 7 天、PWA 用户可能更久不重新授权。方向安全（延迟而非误发），但代价被低估。 | §6.2 写明误报及其代价；指出**正解在上游且很便宜**——请 certus 在状态端点一并返回 `email`（对该客户端不构成新披露：端点已按 consent 限定，该客户端本就从 ID Token 拿到过同一地址），则三字段启发式连同误报可整体删除，改邮箱且新址已验证时也能立即恢复。落地前保持现有 fail-safe 不变，但不当终态设计 |
...
| 2026-08-09 | **R12-UPDATEDAT-FP 闭环** design.md v0.6.0 → **v0.7.0**（conspectus#125，上游 certus#10 已实现于 certus `e432373`）：状态端点现在成对返回 `email + email_verified`，§6.2 的 `updated_at > emailSnapshotIssuedAt` 启发式换成地址直接比对，`emailSnapshotIssuedAt` / `emailSyncRequiredAt` 两字段与 `email_snapshot_stale` 门禁一并删除（迁移 `20260809180000`）。误报路径消失：certus 侧改显示名不再静默停掉该用户的邮件通知；用户改邮箱并验证后下一轮投递即恢复，不必等重新登录。新增两个 fail-closed 原因 `identity_email_unavailable`（响应无 `email`，即 certus 过旧或客户端缺 `email` scope）与 `identity_email_conflict`（both 模式下新地址已被占用），两者都是部署侧问题，不作为用户提示。**部署顺序有约束**：certus 必须先升级，否则全部 certus 来源邮件投递会 fail-closed 延迟。 |
